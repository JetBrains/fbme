...
class RichResourceProjection(
    ...
    override fun getData(@NonNls string: String): Any? {
        if (RichEditorDataKeys.RESOURCE.name == string) {
            return instance.rootInstance.declaration
        }
        if (RichEditorDataKeys.DEVICE.name == string) {
            return (instance.rootInstance.declaration as ResourceDeclaration).container
        }
