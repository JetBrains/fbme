...
class AdapterTypeEcoConverter(private val adapter: AdapterTypeDeclaration, fbmeElement: Element) {
    ...
    private fun generateServiceElement(ecoElement: Element) {
        ...
        for (serviceSequenceElement in serviceSequenceElements) {
            ...
            for (serviceTransactionElement in serviceTransactionElements) {
    ...
    private class AdapterServicePrinterGeneric(val myDeclaration: AdapterTypeDeclaration) {
            ...
            for (serviceSequenceElement in serviceSequenceElements) {
                ...
                for (serviceTransactionElement in serviceTransactionElements) {
                    ...
                    val (event, parameters) = run {
                        ...
                        when {
                            name == "request_confirm" && index == 0 ->
                                getEventAndParam(myDeclaration.inputEvents, myDeclaration.inputParameters, usedInputParams, 0, "REQ", "REQD")
                            name == "request_confirm" && index == 1 ->
                                getEventAndParam(myDeclaration.outputEvents, myDeclaration.outputParameters, usedOutputParams, 0, "CNF", "CNFD")
                            name == "indication_response" && index == 0 ->
                                getEventAndParam(myDeclaration.outputEvents, myDeclaration.outputParameters, usedOutputParams, 1, "IND", "INDD")
                            else ->
                                getEventAndParam(myDeclaration.inputEvents, myDeclaration.inputParameters, usedInputParams, 1, "RSP", "RSPD")
                        }
                    }
